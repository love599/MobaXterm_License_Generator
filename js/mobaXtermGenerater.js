/*!
 * MobaXterm License Generator v1.0.0
 * https://github.com/houyuxi012/mobaxterm-keygen
 *
 * Copyright (c) 2024 HouYuXi
...
*/
function generateLicense(userName, versionName, userNum, licenseType) {
    try {
        let [majorVersion, minorVersion] = versionName.split('.');
        majorVersion = parseInt(majorVersion);
        minorVersion = parseInt(minorVersion || '0');

        const licenseStr = `${licenseType}#${userName}|${majorVersion}${minorVersion}#${userNum}#${majorVersion}3${minorVersion}6${minorVersion}#0#0#0#`;
        const encrypted = EncryptBytes(0x787, licenseStr.toBytes());
        return VariantBase64Encode(encrypted).bytesToStr();
    } catch (error) {
